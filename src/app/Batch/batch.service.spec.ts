...
describe('BatchService', () => {
  ...
  it('should get all batches',
    inject([HttpTestingController, BatchService],
      (httpMock: HttpTestingController, bs: BatchService) => {
        ...
        const b1 = new Batch('testing1', 'Revature', 'test1', 'test train1', 'cotrainer', 1, new Date(''), new Date(''), 50, 45, 5);
        const b2 = new Batch('testing2', 'Revature', 'test2', 'test trainer', 'co trainer', 2, new Date(''), new Date(''), 45, 40, 5);
        req.flush([b1, b2]);
      })
  );
  ...
  it('should get a single batch',
    inject([HttpTestingController, BatchService],
      (httpMock: HttpTestingController, bs: BatchService) => {
        let batch: Batch;
        const testBatch = new Batch('testing1', 'Revature', 'test1', 'test train1', 'cotrainer', 1, new Date(''), new Date(''), 50, 45, 5);
        testBatch.batchId = 1;
        ...
        expect(req.request.method).toEqual('GET');
        const testBatch2 = new Batch('testing1', 'Revature', 'test1', 'test train1', 'cotrainer', 1, new Date(''), new Date(''), 50, 45, 5);
    ...
    inject([HttpTestingController, BatchService],
      (httpMock: HttpTestingController, bs: BatchService) => {
        ...
        const testBatch = new Batch('testing1', 'Revature', 'test1', 'test train1', 'cotrainer', 1,
          new Date('2018-10-04'), new Date(''), 50, 45, 5);
        req.flush([testBatch]);
      })
  );
